...
- **Runner DaemonSet pod `Pending` with `didn't have free ports`** — another workload is already binding hostPorts 3000/2220 on that node. The runner-manager creates capacity-request placeholder pods; it does not create the runner process. For the Kubernetes-native BYOC path, keep `services.runner.mainContainer.enabled: true` and remove any legacy/manual runner Deployment that duplicates the DaemonSet runner.
