...
for component in proxy snapshot-manager runnermanager ssh-gateway; do
...
echo "PASS rollouts: four Deployments and runner DaemonSet"
...
bundle_workload_count="$(jq '[.items[] | select(.metadata.labels["app.kubernetes.io/component"] as $c | ["proxy","snapshot-manager","runnermanager","ssh-gateway","runner"] | index($c))] | length' <<<"$workloads")"
(( bundle_workload_count == 5 )) || omc::die "expected five Daytona image-bundle workloads; found $bundle_workload_count"
bundle_mismatches="$(jq --arg bundle "$expected_bundle" \
  '[.items[] | select(.metadata.labels["app.kubernetes.io/component"] as $c | ["proxy","snapshot-manager","runnermanager","ssh-gateway","runner"] | index($c)) | select(.spec.template.metadata.annotations["daytona.io/image-bundle"] != $bundle)] | length' \
  <<<"$workloads")"
...
runners_json="$(curl -fsS --max-time 30 -H "Authorization: Bearer $DAYTONA_API_KEY" "$DAYTONA_API_URL/runners")"
ready_runners="$(jq --arg id "$region_id" --arg name "$REGION_NAME" '
  (if type == "array" then . else (.items // .result // .data // []) end)
  | [.[] | select((.regionId == $id or .region.id == $id or .region == $name) and .state == "ready" and (.unschedulable != true) and (.draining != true))]
  | length' <<<"$runners_json")"
...
live_runner_pod="$(kubectl -n "$NAMESPACE" get pods -l 'app.kubernetes.io/component=runner' -o json \
